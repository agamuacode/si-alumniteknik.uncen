...
Route::group(['middleware' => ['role:admin|gereja|wilayah']], function () {
    ...
    Route::controller(PemudaController::class)->group(function () {
        Route::get('alumni', [PemudaController::class, 'index'])->name('pemuda');
        Route::get('alumni/tambah', [PemudaController::class, 'create'])->name('alumni.tambah')->middleware(['role:admin|gereja']);
        Route::get('alumni/detail/{id}', [PemudaController::class, 'show'])->name('pemuda.detail');
